...
public class MessageService extends HttpCallService{
    ...
    private static final String FRIEND_MSG_SEND_SERVICE_URL = "https://kapi.kakao.com/v1/api/talk/friends/message/default/send";
    private static final String GET_FRIEND_SERVICE_URL = "https://kapi.kakao.com//v1/api/talk/friends";
...
//    MessageService messageService;
    ...
    public String getFriends(String accessToken) {
        ...
        ResponseEntity<String> response = httpRequest(GET_FRIEND_SERVICE_URL, HttpMethod.GET, messageRequestEntity); // 친구목록 필요
        JSONObject jsonData = new JSONObject(response.getBody());
        System.out.println("jsonData = " + jsonData);
        JSONArray receiver = (JSONArray) jsonData.get("elements");
        String target = receiver.getJSONObject(0).get("uuid").toString();
        return target;
    }

    public boolean sendFriendMessage(String accessToken, DefaultMessageDto msgDto) {
        ...
        uuid.put(msgDto.getReceiver_uuids());
        parameters.add("receiver_uuids", uuid.toString());
        parameters.add("template_object", templateObj.toString());
        System.out.println("parameters = " + parameters);
        HttpEntity<?> messageRequestEntity = httpClientEntity(header, parameters);

        ResponseEntity<String> response = httpRequest(FRIEND_MSG_SEND_SERVICE_URL, HttpMethod.POST, messageRequestEntity);
        JSONObject jsonData = new JSONObject(response.getBody());
        System.out.println("jsonData = " + jsonData);
        return true;
    }
